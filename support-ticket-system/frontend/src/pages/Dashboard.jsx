...
export default function Dashboard() {
  ...
  if (ticket.userId !== user?.id) {
    ...
    return () => { socket.off('ticket:created'); socket.off('ticket:updated'); socket.off('ticket:deleted') }
  }, [])
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '16px', marginBottom: '32px' }}>
          <div>
            ...
            <p style={{ color: '#9ca3af', marginTop: '6px', fontSize: '14px' }}>Here's what's happening with your tickets</p>
          </div>

          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
            {weather && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px 14px', borderRadius: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', fontSize: '13px', color: '#d1d5db' }}>
                <Globe size={14} color="#6366f1" />
                {getWeatherIcon(weather.weathercode)} {Math.round(weather.temperature)}°C · Hyderabad
              </div>
            )}
            <button onClick={fetchTickets}
              style={{ padding: '8px', borderRadius: '10px', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)', cursor: 'pointer', color: '#9ca3af', display: 'flex', alignItems: 'center' }}>
              <RefreshCw size={16} />
            </button>
            <button onClick={() => setShowModal(true)}
              style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '9px 18px', borderRadius: '10px', background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', border: 'none', color: 'white', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '24px' }}>
          {['ALL', 'OPEN', 'CLOSED'].map(f => (
            <button key={f} onClick={() => setFilter(f)}
              style={{
                padding: '7px 16px', borderRadius: '10px', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '500',
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px 0' }}>
            ...
            <p style={{ color: '#9ca3af', fontSize: '16px', margin: '0 0 6px' }}>No tickets found</p>
            <p style={{ color: '#6b7280', fontSize: '13px', margin: 0 }}>
              {filter === 'ALL' ? 'Create your first ticket!' : `No ${filter.toLowerCase()} tickets`}
            </p>
            {filter === 'ALL' && (
              <button onClick={() => setShowModal(true)}
                style={{ marginTop: '16px', padding: '10px 24px', borderRadius: '10px', background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', border: 'none', color: 'white', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>
                Create First Ticket
              </button>
            )}
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '16px' }}>
            {filtered.map(ticket => (
              <TicketCard key={ticket.id} ticket={ticket} onDelete={handleDelete} onStatusChange={handleStatusChange} />
            ))}
          </div>
        )}
      </div>

      {showModal && <CreateTicketModal onClose={() => setShowModal(false)} onCreated={(t) => setTickets(prev => [t, ...prev])} />}
